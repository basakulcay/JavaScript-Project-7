...
//Check through nested arroy of cards, return invalid: 
...
function findInvalidCardCompanies(invalidCards) {
  ...
  for (let j=0; j<invalidCards.length; j++) {
    ...
    if(inv[0]===3) {invalidCompany.push('Amex')}
    else if (inv[0]===4) {invalidCompany.push('Visa')}
    else if (inv[0]===5) {invalidCompany.push ('Mastercard')}
    else if (inv[0]===6) {invalidCompany.push('Discover')}
